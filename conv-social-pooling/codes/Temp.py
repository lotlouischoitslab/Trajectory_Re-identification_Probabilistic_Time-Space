...
def predict_trajectories(input_data, overpass_start_loc_x, overpass_end_loc_x, fut_pred, batch_num, delta, alpha):
    ...
    for counter,ids in enumerate(modified_IDs_to_traverse):
        ...
        for poss_counter,traj_id in enumerate(traj_to_check_IDs):   
            print(f'Counter: {counter+1}/{len(modified_IDs_to_traverse)} | Poss_Counter: {poss_counter+1}/{len(traj_to_check_IDs)}')
            segment_integral = 0 
            ...
            if len(y_list) > 1 and y_list[1] - y_list[0] > 0:
            ...
            for m in range(num_maneuvers):
                ...
                mux_store_temp = [mx + overpass_start_loc_x for mx in muX_before]
    ...
    temp_vehicle_1 = pd.read_csv('details/trajectory_details.csv') 
    print(temp_vehicle_1.head())
    temp_vehicle_1_temp = temp_vehicle_1[temp_vehicle_1['Vehicle_ID'] == 1]
    pd.DataFrame(temp_vehicle_1_temp).to_csv('details/vehicle_1.csv', index=False)
...
def evaluate_trajectory_prediction():   
    ...
    for vehicle_id, group in grouped_df:
        ...
        for idx, row in group.iterrows():
            if row['ID'] == row['Vehicle_ID'] and row['line_integral_values'] == max_integral_value:
...
def main(): # Main function 
    ...
    file_to_read = 'ngsim_data.csv'  # NGSIM csv dataset 
    df = pd.read_csv(file_to_read) # read in the data 

    columns_order = ['Dataset_ID', 'ID', 'Frame_ID', 'xloc', 'yloc', 'lane', 'lateral_man', 'longitudinal_man'] + [f'neighbor_{i}' for i in range(1, 40)]

    df.columns = columns_order

    df.to_csv('details/ngsim_data.csv', index=False) # save the dataframe to
    ...
    model_path = 'ngsim_trained_model/cslstm_m.tar' # NGSIM model
