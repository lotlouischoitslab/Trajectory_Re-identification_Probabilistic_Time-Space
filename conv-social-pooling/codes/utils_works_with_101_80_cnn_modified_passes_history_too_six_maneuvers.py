...
class ngsimDataset(Dataset):
    ...
    def collate_fn(self, samples):
        ...
        points_batch = torch.zeros(len(samples),49)
        maneuver_enc_batch = torch.zeros(len(samples), 6)


        count = 0
        for sampleId,(hist, fut, nbrs, lat_enc, lon_enc, points, maneuver_enc) in enumerate(samples):
            ...
            points_batch[sampleId,:] = torch.from_numpy(points)
        ...
        return hist_batch, nbrs_batch, mask_batch, lat_enc_batch, lon_enc_batch, fut_batch, op_mask_batch ,points_batch, maneuver_enc_batch
